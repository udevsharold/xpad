#import "XPPManageShortcutsController.h"
#import "XPPCustomActionViewController.h"
#import "XPPKeyboardTypeOptions.h"
#import "XPPSnippetEntryController.h"
#import "XPPInsertTextEntryController.h"
...
#import "XPPPasteOptions.h"
#import "XPPSpongebobOptions.h"

static BOOL translomaticInstalled = NO;
static BOOL tranzloInstalled = NO;

@implementation XPPManageShortcutsController
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    switch(indexPath.section) {
        case 0: {
            ...
            label = [XPHelper labelFromArray:self.currentOrder[0] atIndex:indexPath.row];
            image = [XPHelper imageFromArray:self.currentOrder[0] atIndex:indexPath.row withSystemColor:YES completion:nil];
            break;
        }
        case 1: {
            ...
            label = [XPHelper labelFromArray:self.currentOrder[1] atIndex:indexPath.row];
            image = [XPHelper imageFromArray:self.currentOrder[1] atIndex:indexPath.row withSystemColor:YES completion:nil];
            ...
            label = [XPHelper labelFromArray:self.extrasOptions atIndex:indexPath.row];
            image = [XPHelper imageFromArray:self.extrasOptions atIndex:indexPath.row withSystemColor:YES completion:nil];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section < 2){
        XPPGesturePickerController *gesturePickerController = [[XPPGesturePickerController alloc] init];
        ...
        if ([self.extrasOptions[indexPath.row][@"identifier"] isEqualToString:@"keyboardType"]){
            XPPKeyboardTypeOptions *kbTypeOptions = [[XPPKeyboardTypeOptions alloc] init];
            ...
            XPPSnippetEntryController *snippetEntryController = [[XPPSnippetEntryController alloc] init];
            ...
            XPPInsertTextEntryController *insertTextController = [[XPPInsertTextEntryController alloc] init];
            ...
            XPPCursorMoveAndSelectEntryController *cursorMoveAndSelectController = [[XPPCursorMoveAndSelectEntryController alloc] init];
            ...
            XPPDeleteOptions *deleteOptions = [[XPPDeleteOptions alloc] init];
            ...
            XPPGlobeOptions *globeOptions = [[XPPGlobeOptions alloc] init];
            ...
            XPPPasteOptions *pasteOptions = [[XPPPasteOptions alloc] init];
            ...
            XPPSpongebobOptions *spongebobOptions = [[XPPSpongebobOptions alloc] init];
...
-(void)updatedFromOldVersion:(BOOL)fromOldVersion{
    if (fromOldVersion){
        UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"XPXPad" message:@"You just updated to a major version, shortcuts settings will be reset for compatibility." preferredStyle:UIAlertControllerStyleAlert];
...
- (void)updateOrder:(BOOL)reset {
    NSMutableDictionary *prefs = [[[XPPrefsManager sharedInstance] readPrefs] mutableCopy] ?: [NSMutableDictionary dictionary];
    
    /*
    BOOL updatedFromOldVersion = ![prefs[@"updatedFromV1"] boolValue];
    if (updatedFromOldVersion){
        reset = YES;
        [[XPPrefsManager sharedInstance] setValue:@YES forKey:@"updatedFromV1"];
    ...
    if (reset){
        ...
        [[XPPrefsManager sharedInstance] writePrefs:prefs];
        
        //Remove all caches
        NSFileManager *fm = [NSFileManager defaultManager];
        for (NSString *cacheFile in [fm contentsOfDirectoryAtPath:XPadCachePath error:nil]) {
            [fm removeItemAtPath:[NSString stringWithFormat:@"%@/%@", XPadCachePath, cacheFile] error:nil];
        }
    }
...
-(void)reset{
    ...
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"XPXPad" message:@"Reset all shortcuts included customized long press and double tap actions to default?" preferredStyle:UIAlertControllerStyleAlert];
