...
@implementation XPPCustomActionViewController
...
- (NSString *)tableView:(UITableView *)tableView titleForFooterInSection:(NSInteger)section
{
    switch (section) {
        case 0:
            return @"This action will be performed prior to the second action below. It can be none. Leave all blank for default actions.";
        default:
            return @"";
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    switch(indexPath.section) {
        case 0: {
            ...
            label = [XPHelper labelFromArray:self.fullOrder[0] atIndex:indexPath.row];
            image = [XPHelper imageFromArray:self.fullOrder[0] atIndex:indexPath.row withSystemColor:YES completion:^(BOOL thirteen, BOOL customPath){
                isThirteen = thirteen;
        ...
        case 1: {
            ...
            label = [XPHelper labelFromArray:self.fullOrder[1] atIndex:indexPath.row];
            image = [XPHelper imageFromArray:self.fullOrder[1] atIndex:indexPath.row withSystemColor:YES completion:^(BOOL thirteen, BOOL customPath){
                isThirteen = thirteen;
                isCustomImagePath = customPath;
                dispatch_semaphore_signal(smp);
            }];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    [[XPPrefsManager sharedInstance] writePrefs:self.prefs];
...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.prefs = [[[XPPrefsManager sharedInstance] readPrefs] mutableCopy];
    ...
    self.defaultBtn = [[UIBarButtonItem alloc] initWithTitle: @"Default" style:UIBarButtonItemStylePlain target:self action:@selector(resetToDefault)];
